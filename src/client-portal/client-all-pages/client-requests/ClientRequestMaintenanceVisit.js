import React from 'react'

const ClientRequestMaintenanceVisit = () => {
    return (
        <>
            <div className="lrm-content">
  <div className="row align-items-center">
    <div className="col-lg-5">
      <h4 className="small-title">Request Maintenance Visit <span className="mini-dot"></span>
        <a href="#" className="ml-2 subs">List of Scheduled Visits</a>
      </h4>
    </div>
    <div className="col-lg-7">
      <div className="search-flex">
        <input type="text" name="" placeholder="Search" className="form-control" />
        <button className="btn btn-primary"><i className="fa fa-search"></i> </button>
         <a href="#" className="btn btn-primary add">New Request</a>
       
      </div>
    </div>
  </div>
  <div className="card mt-3">
    <div className="table-responsive">
       <table className="table">
          <thead>
            <tr>
              <th scope="col">Venue Name</th>
              <th scope="col">Address</th>
              <th scope="col">City</th>
              <th scope="col">Request made on</th>  
              <th scope="col">Subject</th>   
              <th scope="col">Description</th>   
              <th scope="col">Time</th>   
              <th scope="col">Date Request</th>   
              <th scope="col">Status</th> 
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
            <tr>
              <td>Abu Dhabi Cricket #3</td>
              <td>2331 Villager Park Ct</td>
              <td>Mexico City</td>
              <td>dd/mm/yyyy</td>
              <td>Some subject</td>
              <td>Some Description</td>
              <td>09:50</td>
              <td>dd/mm/yyyy</td>
              <td><span className="badge badge-yellow">Active</span> </td>                              
            </tr>
       
          </tbody>
        </table>
      </div>
  </div>
</div>

{/* <!-- Modal --> */}
<div className="modal fade delete-modal" id="delete-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div className="modal-dialog modal-dialog-centered" role="document" style={{maxWidth: '450px'}}>
    <div className="modal-content">
      <div className="modal-body">
        <h5>Before you proceed!</h5>
        <p>Are you sure you want to delete this venue?</p>
        <div className="bottom">
          <button type="button" className="btn btn-primary">Yes</button>
          <button type="button" className="btn btn-outline-primary" data-dismiss="modal">No</button>
        </div>
      </div>
    </div>
  </div>
</div>
        </>
    )
}

export default ClientRequestMaintenanceVisit
